...
def run():
    ...
    print(
        "Welcome to the profile populator. If, at any step, you'd prefer not to "
        "enter the requested information, just hit 'Enter' with a blank field to continue."
    )
    ...
    # gmail
    print(
        "\nJasper uses your Gmail to send notifications. Alternatively, you can "
        "skip this step (or just fill in the email address if you want to receive email notifications) "
        "and setup a Mailgun account, as at http://jasperproject.github.io/documentation/software/#mailgun.\n"
    )
    ...
    print(
        "Enter one of the following: 'AT&T', 'Verizon', 'T-Mobile' OR go "
        "to http://www.emailtextmessages.com and enter the email suffix for "
        "your carrier (e.g., for Virgin Mobile, enter 'vmobl.com')."
    )
    ...
    print(
        "Location should be a 5-digit US zipcode (e.g., 08544). If you are outside the US, "
        "insert the name of your nearest big town/city. For weather requests.")
    location = raw_input("Location: ")
    while location and verifyLocation(location):
    ...
    # timezone
    print(
        "Please enter a timezone from the list located in the TZ* column "
        "at http://en.wikipedia.org/wiki/List_of_tz_database_time_zones, or none at all."
    )
